...
<%	
	...
	//从控制层获得列表，在该页面显示
	//取边左边显示的最新新闻的前五条
	Map<String, List<News>> sideNewsMap = (Map<String, List<News>>) session.getAttribute("sideNewsMap");//获得两组新闻
	List<Topic> topicList = (List<Topic>) session.getAttribute("topicList"); //获得首页主题
	...
	//4.获得主题下的新闻
		List<News> topicNewsList = (List<News>) session.getAttribute("topicNewsList");
	
	//5.获得默认的新闻
		List<News> defaultNewsList = (List<News>) session.getAttribute("defaultNewsList");
	
	//6.获得分页后的所有新闻
		List<News> allPageNewsList = (List<News>) session.getAttribute("newsListPage"); 
	
	//7.获得当前页码
		Integer pageIndexInt = (Integer) session.getAttribute("pageIndex");
		int pageIndex = 1;
	...
	//7-2.获得当前页码
		Integer pageIndexInt2 = (Integer) session.getAttribute("pageIndex2");
		int pageIndex2 = 1;
		if(pageIndexInt2 != null){
			pageIndex2 = pageIndexInt2;
		}
	//8-2.获得总页数
		Integer pageCount2 = (Integer) session.getAttribute("pageCount2");
	
	
	//9.获得当前页面主题编号
		Integer ntidInt = (Integer)session.getAttribute("ntid");
		int ntid = 1;
		if(ntidInt != null){
			ntid = ntidInt;
		}
...
<div id="header">
  <div id="top_login">
    <form action="util/do_login?opr=index" method="post" onsubmit="return check()">
      <label> 登录名 </label>
...
<div id="container">
    ...
    <div class="content">
      <ul class="class_date">
       <!--  <li id='class_month'> <a href='#'><b> 国内 </b></a> <a href='#'><b> 国际 </b></a> <a href='#'><b> 军事 </b></a> <a href='#'><b> 体育 </b></a> <a href='#'><b> 娱乐 </b></a> <a href='#'><b> 社会 </b></a> <a href='#'><b> 财经 </b></a> <a href='#'><b> 科技 </b></a> <a href='#'><b> 健康 </b></a> <a href='#'><b> 汽车 </b></a> <a href='#'><b> 教育 </b></a> </li>
        ...
        <li id='class_month'>      
        	...
        	<a href='util/news_control?ntid=<%=topicList.get(i).getTid()%>&opr=topicNews'><b> <%=topicList.get(i).getTname() %> </b></a>
      ...
      <ul class="classlist">
      	...
      	%>
       	...
       	<li><a href='util/news_control?opr=readNews&nid=<%= news.getNid() %>'> <%=news.getNtitle() %> </a><span> <%=news.getNcreateDate() %> </span></li>
       ...
       <%-- 	<%	//默认新闻
       	 if(topicNewsList == null){
      		if(defaultNewsList != null){
      			for(int i = 0;i<defaultNewsList.size() ; i++){
      				News deafultNews = defaultNewsList.get(i);
      	%>
      
       	<li><a href='util/news_control.jsp?opr=readNews&nid=<%= deafultNews.getNid() %> '> <%=deafultNews.getNtitle() %> </a><span> <%=deafultNews.getNcreateDate() %> </span></li>
      	...
      	%>
       	...
       	<li><a href='util/news_control?opr=readNews&nid=<%= allNewsPageList.getNid() %> '> <%=allNewsPageList.getNtitle() %> </a><span> <%=allNewsPageList.getNcreateDate() %> </span></li>
        ...
        //显示默认新闻的上下页
      	...
      	%>
				...
				<a href="util/news_control?pageIndex=1&opr=list">首页</a>
	      	    <a href="util/news_control?pageIndex=<%=pageIndex - 1%>&opr=list">上一页</a>
			<%
			} if (pageIndex < pageCount) { // 控制页面显示风格
			%>
			   <a href="util/news_control?pageIndex=<%=pageIndex + 1%>&opr=list">下一页</a>
      	   	   <a href="util/news_control?pageIndex=<%=pageCount %>&opr=list">末页</a>
		      	     
       <%-- 	 <p align="right">
           <a href="util/news_control.jsp?pageIndex=1&opr=list">首页</a>
      	   <a href="util/news_control.jsp?pageIndex=<%=pageIndex - 1%>&opr=list">上一页</a>
           <a href="util/news_control.jsp?pageIndex=<%=pageIndex + 1%>&opr=list">下一页</a>
      	   <a href="util/news_control.jsp?pageIndex=<%=pageCount %>&opr=list">末页</a>
          </p> --%>
       	...
       	%>	
			...
			%>   
			  <a href="util/news_control?pageIndex2=1&opr=topicNews&ntid=<%=ntid%>">首页</a>
      	   	  <a href="util/news_control?pageIndex2=<%=pageIndex2 - 1%>&opr=topicNews&ntid=<%=ntid%>">上一页</a>
			<%
			} if (pageIndex2 < pageCount2) { // 控制页面显示风格
			%>
			   <a href="util/news_control?pageIndex2=<%=pageIndex2 + 1%>&opr=topicNews&ntid=<%=ntid%>">下一页</a>
      	       <a href="util/news_control?pageIndex2=<%=pageCount2 %>&opr=topicNews&ntid=<%=ntid%>">末页</a>
			<%   
			} }
			%>   
         ...
         <%--  <p align="right">
           <a href="util/news_control.jsp?pageIndex=1&opr=topicNews&ntid=<%=ntid%>">首页</a>
      	   <a href="util/news_control.jsp?pageIndex=<%=pageIndex - 1%>&opr=topicNews&ntid=<%=ntid%>">上一页</a>
           <a href="util/news_control.jsp?pageIndex=<%=pageIndex + 1%>&opr=topicNews&ntid=<%=ntid%>">下一页</a>
      	   <a href="util/news_control.jsp?pageIndex=<%=pageCount %>&opr=topicNews&ntid=<%=ntid%>">末页</a>
          </p> --%>
  ...
  <%
  	//session在服务器持续很长时间，影响服务器性能
  	session.removeAttribute("sideNewsMap");
  	session.removeAttribute("topicList");
  	session.removeAttribute("topicNewsList");
  	session.removeAttribute("defaultNewsList");

  	session.removeAttribute("pageAllNewsList");
  	//session.removeAttribute("topicNewsList");先不要删除掉了，删掉以后，一刷新就没了
  %>
