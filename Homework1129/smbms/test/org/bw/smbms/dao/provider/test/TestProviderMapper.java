...
//    }
    ...
    public void testFindProviderList(){
        ...
        try {
            ...
            List<Provider> providerList = session.getMapper(ProviderMapper.class).findProviderList((pageCount-1)*pageSize,pageSize);
    ...
    @Test
    public void testFindProviderByProCodeAndProNameAndProContactAndCreationDate(){
        SqlSession session = MyBatisUtil.sqlSessionFactory.openSession();

        try {

            Map<String,Object> providerMap = new HashMap<String,Object>();
            providerMap.put("proCode","0");
            providerMap.put("proName","司");
            providerMap.put("proContact","王");
            providerMap.put("companyAge","中公司");

            List<Provider> providerList = session.getMapper(ProviderMapper.class).findProviderByProCodeAndProNameAndProContactAndCreationDate(providerMap);
            for (Provider provider:providerList){
                logger.info(provider);
            }
        }catch (Exception e){
